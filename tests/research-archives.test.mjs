...
test("preserved 0.18.0 installers match the exact public release inventory", async () => {
  ...
  for (const artifact of manifest.artifacts) {
    ...
    assert.match(artifact.path, /^windows-x64\/[^/]+\.exe$/);
...
test("bootstrap prefers the hash-pinned local Windows archive before the network", async () => {
  const [attributes, config, bootstrap] = await Promise.all([
    ...
    readFile(path.join(repositoryRoot, "scripts", "bootstrap-windows-runtime.mjs"), "utf8"),
  ]);
  assert.match(attributes, /research-archives\/original\/\*\*\/\*\.exe filter=lfs diff=lfs merge=lfs -text/);
  assert.match(config, /export const archivedWindowsInstaller = path\.join\(repoRoot, "research-archives", "original", "0\.18\.0", "windows-x64", "Grok_Bot_0\.18\.0_Setup\.exe"\)/);
  assert.match(bootstrap, /const digest = await sha256\(archivedWindowsInstaller\)/);
  assert.match(bootstrap, /if \(digest !== windowsInstallerSha256\)/);
  assert.match(bootstrap, /await copyFile\(archivedWindowsInstaller, cachedWindowsInstaller\)/);
  assert.ok(bootstrap.indexOf("await copyFile(archivedWindowsInstaller, cachedWindowsInstaller)") < bootstrap.indexOf("await fetch(windowsInstallerUrl"));
});
