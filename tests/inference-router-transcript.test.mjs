...
test("routed transcript merge is chronological and normalizes colliding ids", async () => {
  const loaded = await loadModule();
  try {
    const merged = loaded.module.mergeInferenceRouterTranscriptEntries(
      [
        { id: "t0u", timestampMs: 300, content: "new remote" },
        { id: "t1s0", timestampMs: 400, content: "latest" },
      ],
      [
        { id: "t0u", timestampMs: 100, content: "old local" },
        { id: "t0s0", timestampMs: 200, content: "middle" },
      ],
      10,
    );
    assert.deepEqual(merged.map(entry => entry.content), ["old local", "middle", "new remote", "latest"]);
    assert.equal(merged[0].id, "t0u");
    assert.equal(merged[2].id, "t0u~300~1");
  } finally {
    await loaded.dispose();
  }
});
