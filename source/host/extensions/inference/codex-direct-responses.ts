...
export type CodexDirectEvent =
  ...
  | { readonly type: "done"; readonly text: string; readonly responseId: string; readonly usage: CodexDirectUsage; readonly toolCalls?: readonly CodexDirectToolCall[] };

export type CodexDirectToolCall = {
...
function toolCallArguments(call: Loose): unknown {
...
/**
 * Converts the Agent's replay messages into native Responses API items. Tool
 * calls and results must retain their call ids across inference turns; sending
 * those structured parts as ordinary JSON text prevents Codex from continuing
 * the Grok Bot tool loop.
 */
export function codexDirectInput(messages: readonly { readonly role: string; readonly content: string | readonly unknown[] }[]): Loose[] {
  const input: Loose[] = [];
  for (const message of messages) {
    ...
    let emittedStructuredPart = false;
    const text: string[] = [];
    for (const rawPart of message.content) {
      ...
      if (
        ...
        if (text.length > 0) {
          input.push({ role: "assistant", content: text.splice(0).join("") });
        }
        input.push({
          type: "function_call",
          call_id: part.toolCallId,
          name: part.toolName,
          arguments: safeJson(part.args ?? {}),
        });
        emittedStructuredPart = true;
        continue;
      }
      if (
        message.role === "tool"
        ...
        input.push({
          ...
          call_id: part.toolCallId,
          output: safeJson(part.isError === true
            ? { isError: true, error: part.result }
            : part.result),
        });
        emittedStructuredPart = true;
      }
    ...
    if (text.length > 0) {
      input.push({ role: message.role === "assistant" ? "assistant" : "user", content: text.join("") });
      emittedStructuredPart = true;
    }
    if (!emittedStructuredPart) {
      input.push({ role: message.role === "assistant" ? "assistant" : "user", content: safeJson(message.content) });
    }
  }
  return input;
}
...
export async function* streamCodexDirectResponses(options: CodexDirectOptions): AsyncGenerator<CodexDirectEvent> {
  ...
  for (let step = 0; step < maxSteps; step += 1) {
    ...
    if (options.executeTool == null) {
      const forwardedCalls = calls.map((call): CodexDirectToolCall => ({
        toolCallId: call.call_id,
        toolName: call.name,
        args: toolCallArguments(call),
      }));
      for (const call of forwardedCalls) yield { type: "tool-call", ...call };
      yield { type: "done", text, responseId, usage, toolCalls: forwardedCalls };
      return;
    }
