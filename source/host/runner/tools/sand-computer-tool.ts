...
function throwComputerError(result: ComputerUseResult): void {
  if (result.result.case === "error") {
    const value = result.result.value as { readonly error?: string };
    throw new Error(value.error || "Computer action failed.");
  }
}

/** Agent-native wrapper for the low-level computer-use bridge. */
export function createCommunicateComputerTool(deps: ComputerToolDependencies<unknown>) {
  // Direct computer use is scoped to the user's isolated box. The legacy
  // review preflight cannot describe a request until a desktop page exists,
  // so keeping it here makes first-use control impossible.
  const { autoReview: _legacyAutoReview, ...directDependencies } = deps;
  const core = createComputerTool(directDependencies);
  return defineCommunicateTool<ComputerActionArgs, { readonly core: typeof core }>(
    { core },
    {
      id: "OPENAI_COMPUTER_USE",
      name: "Computer",
      description: "Control the box desktop with mouse, keyboard, scrolling, waits, and screenshots.",
      parameters: core.parameters,
      describeActivity: (args) => ({
        detail: args.action,
        ...(args.description === undefined ? {} : { target: args.description }),
      }),
      async execute(context, args, { core: tool, toolCallId, signal, stateHandler, workspacePaths }) {
        const result = await tool.execute(args, {
          context,
          toolCallId,
          ...(signal === undefined ? {} : { signal }),
          ...(stateHandler === undefined ? {} : { stateHandler }),
          ...(workspacePaths === undefined ? {} : { workspacePaths }),
        });
        throwComputerError(result);
        return describeOutcome(result, "computer");
      },
    },
  );
}

/** Agent-native wrapper for the standalone desktop screenshot action. */
export function createCommunicateScreenshotTool(deps: ComputerToolDependencies<unknown>) {
  const core = createScreenshotTool(deps);
  return defineCommunicateTool<Record<string, never>, { readonly core: typeof core }>(
    { core },
    {
      id: "OPENAI_COMPUTER_USE_SCREENSHOT",
      name: "Screenshot",
      description: "Capture the current box desktop so you can inspect what is visible.",
      parameters: screenshotParameters,
      async execute(context, args, { core: tool, toolCallId }) {
        const result = await tool.execute(args, { context, toolCallId });
        throwComputerError(result);
        return describeOutcome(result, "screenshot");
      },
    },
  );
}
