...
export function createSandBrowserTools<Context>(
  dependencies: BrowserDriverDependencies<Context> & {
    ...
    readonly captureActionScreenshots?: boolean;
  },
  ...
  return BROWSER_TOOL_SPECS.map((spec) => ({
    ...
    async execute(context, args, metadata) {
      try {
        ...
        const skipScreenshot = spec.skipScreenshot === true
          || (dependencies.captureActionScreenshots === false && spec.op !== "screenshot");
...
const browserArgumentFields: Readonly<Record<string, ZodTypeAny>> = {
...
function browserParameters(schema: BrowserToolSchema): ZodTypeAny {
  const required = new Set(schema.required ?? []);
  const shape: Record<string, ZodTypeAny> = {};
  for (const [key, value] of Object.entries(browserArgumentFields)) {
    shape[key] = required.has(key) ? value : value.optional();
  }
  return z.object(shape);
}

/** Adapts the browser driver to the streamed Agent tool contract used by the
 * host runner and by direct Codex tool replay. */
export function createSandBrowserAgentTools(
  dependencies: BrowserDriverDependencies<unknown> & {
    readonly onPossibleNavigation?: (context: unknown) => void;
  },
) {
  // This adapter is used only for the user's own isolated box. The legacy
  // Auto-review preflight requires an already-running Chrome page and would
  // otherwise block the very first navigation that starts Chrome.
  const { autoReview: _legacyAutoReview, ...directDependencies } = dependencies;
  // The communicate adapter returns text, not the per-action bitmap. Avoid
  // rendering, transferring, and persisting an invisible screenshot after
  // every click/navigation; the explicit screenshot tool still captures one.
  return createSandBrowserTools({
    ...directDependencies,
    captureActionScreenshots: false,
  }).map((browserTool) =>
    defineCommunicateTool<Record<string, unknown>, { readonly browserTool: BrowserToolDefinition<unknown> }>(
      { browserTool },
      {
        id: browserTool.id,
        name: browserTool.name,
        description: browserTool.description,
        parameters: browserParameters(browserTool.schema),
        describeActivity: (args) => {
          const target = typeof args.url === "string"
            ? args.url
            : typeof args.ref === "string" ? args.ref : undefined;
          return {
            detail: browserTool.op,
            ...(target === undefined ? {} : { target }),
          };
        },
        async execute(context, args, { browserTool: tool, toolCallId, stateHandler, workspacePaths }) {
          const output = await tool.execute(context, args, {
            toolCallId,
            ...(stateHandler === undefined ? {} : { stateHandler }),
            ...(workspacePaths === undefined ? {} : { workspacePaths }),
          });
          if (output.isError === true) throw new SandBrowserDriverError(output.text);
          return output.text;
        },
      },
    )
