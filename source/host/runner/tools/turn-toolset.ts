...
import {
  createCommunicateComputerTool,
  createCommunicateScreenshotTool,
  type ComputerToolDependencies,
} from "./sand-computer-tool.js";
import {
  createSandBrowserAgentTools,
...
export function createTurnComputerToolFactory(
  ...
  return () => asTurnTool(createCommunicateComputerTool(input.dependencies));
}

export function createTurnScreenshotToolFactory(
  input: TurnComputerToolFactoryInput,
): () => TurnTool {
  return () => asTurnTool(createCommunicateScreenshotTool(input.dependencies));
}

export function createTurnBrowserToolFactory(
  input: TurnBrowserToolFactoryInput,
): () => readonly TurnTool[] {
  return () => createSandBrowserAgentTools(input.dependencies).map(asTurnTool);
}
