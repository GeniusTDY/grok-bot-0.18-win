...
export function createTurnSettle(
  ...
  let baseStateForTranscriptRecovery: TurnCheckpoint | undefined;
  let transcriptMirrorRecovered = false;
  ...
  function noteBaseState(
    ...
    baseStateForTranscriptRecovery = baseState;
  }
  ...
  async function persistCheckpoint(
    ...
    if (preparedTranscriptMirror != null) {
      if (!transcriptMirrorRecovered) {
        if (baseStateForTranscriptRecovery === undefined) {
          throw new TypeError("turn base state is not bound for transcript recovery");
        }
        await preparedTranscriptMirror.recover(
          context,
          host.getTranscriptId(),
          baseStateForTranscriptRecovery,
          host.getBlobStore(),
        );
        transcriptMirrorRecovered = true;
      }
