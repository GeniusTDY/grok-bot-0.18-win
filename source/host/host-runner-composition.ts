  ...
  function createRunner(
    ...
    const createTurnToolProjections =
      autoReviewGate == null
        ...
        : (input: ProductionTurnToolInputs): ProductionTurnHostToolProjections => {
          const createProjectionShell = (
            resourceAccessor: ProductionTurnToolInputs["resourceAccessor"],
          ) => createHostShellExecutor({
              resourceAccessor,
          ...
          const projection = {
            createComputerToolDependencies: (projectionInput: ProductionTurnToolInputs) => createHostComputerToolDependencies({
              resourceAccessor: projectionInput.resourceAccessor,
              autoReview: {
            ...
            createScreenshotToolDependencies: (projectionInput: ProductionTurnToolInputs) => createHostComputerToolDependencies({
              resourceAccessor: projectionInput.resourceAccessor,
              ...(persistImageForTurn === undefined
                ? {}
                : { persistImage: persistImageForTurn }),
            ...
            createBrowserDriverDependencies: (projectionInput: ProductionTurnToolInputs) => createHostBrowserDriverDependencies({
              resourceAccessor: projectionInput.resourceAccessor,
              box: remoteBox as unknown as HostBrowserBoxOwner<unknown>,
              getBoxId: () => session.id,
              getDefaultViewId: () => session.id,
              executeShell: createProjectionShell(projectionInput.resourceAccessor),
              autoReview: {
                mode: projectionAutoReviewModes.computer,
                agentId: session.id,
                boxIdentity: {
            ...
            createBoxShellExecutor: (projectionInput: ProductionTurnToolInputs) =>
              createProjectionShell(projectionInput.resourceAccessor),
          };
          return projection;
        };
    ...
    const productionPromptGlue = productionContext === undefined
      ...
      : (() => {
        ...
        if (remoteBoxForPrompt === undefined) return undefined;
        const box = asTransferBox(localExec.box) ?? remoteBoxForPrompt;
    ...
    const createTurnToolsetFactoryProvider = (
      ...
      createComputerToolInputs: (turn, props): TurnComputerToolFactoryInput => {
        ...
        const remoteProps: ProductionTurnToolInputs = {
          ...props,
          resourceAccessor: turn.remoteBoxResourceAccessor,
        };
        const createDependencies = props.createComputerToolDependencies
          ?? createTurnToolProjections?.(remoteProps).createComputerToolDependencies;
        if (createDependencies === undefined) {
          throw new TypeError("computer tool dependencies are not bound");
        }
        return { dependencies: createDependencies(remoteProps) };
      },
      createBrowserToolInputs: (turn, props): TurnBrowserToolFactoryInput => {
        ...
        const createDependencies = props.createBrowserDriverDependencies
          ?? createTurnToolProjections?.(remoteProps).createBrowserDriverDependencies;
        if (createDependencies === undefined) {
          throw new TypeError("browser driver dependencies are not bound");
        }
        return { dependencies: createDependencies(remoteProps) };
      },
      createScreenshotToolInputs: (turn, props): TurnComputerToolFactoryInput => {
        ...
        const createDependencies = props.createScreenshotToolDependencies
          ?? createTurnToolProjections?.(remoteProps).createScreenshotToolDependencies;
        if (createDependencies === undefined) {
