...
function transcriptTimestamp(entry: unknown): number {
  ...
  return {
    ...
    async dispatch(method: string, args: unknown): Promise<{ handled: boolean; value?: unknown }> {
      ...
      if (!usesNativeCliProxyHost && provider !== "cursor" && ["getAgentTranscriptTail", "openAgentTail", "getAgentTranscriptWindow"].includes(method)) {
        ...
        const entries = mergeInferenceRouterTranscriptEntries(
          result.entries,
          (local.agents[agentId] ?? []).map(projectInferenceRouterTranscriptEntry),
          limit,
        );
        return { handled: true, value: { ...result, entries } };
      }
      // Codex credentials are mounted read-only into the local VM host. Let the
      // production host runner own Codex turns so it can publish and execute its
      // full box/browser/computer toolset. The coordinator-local route only has
      // access to desktop-routed MCP tools and would otherwise reduce Codex to a
      // text-only assistant.
      if (method !== "sendPrompt" || provider === "cursor" || provider === "codex") return { handled: false };
