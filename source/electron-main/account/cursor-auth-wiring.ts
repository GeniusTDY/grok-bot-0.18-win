...
export function createCursorAuthWiring(deps: {
  ...
  async function syncLocalToolPermissionCeiling(service: AuthServicePort, status: SandAuthStatus): Promise<void> {
    ...
    if (status.kind === "logged-in" && status.localProvider == null) ceiling = await readLocalToolPermissionCeiling((options) => service.getValidAccessToken(options));
  ...
  function deliverCursorAuthStatus(service: AuthServicePort, status: SandAuthStatus): void {
    ...
    if (deps.sentryEnabled && (status.kind !== "logged-in" || status.localProvider == null)) void syncSentryAccount(status, () => readPrivacyMode((options) => service.getValidAccessToken(options)));
...
export function createCursorAccountEdgePort(deps: {
  ...
  const sandAccessDeps = async () => {
    ...
    return {
      getAuthStatus: () => settledStatus(() => service.getStatus()),
      readAccess: async () => {
        const status = await settledStatus(() => service.getStatus());
        return status.kind === "logged-in" && status.localProvider != null
          ? { state: "granted" as const, reason: "none" as const }
          : await deps.readSandAccess((options) => service.getValidAccessToken(options));
      }
    };
  };
  ...
  return {
    ...
    getAvatar: async () => withService(async (service) => { const status = await service.getStatus(); return status.kind !== "logged-in" || status.authId == null || status.localProvider != null ? null : await deps.resolveAvatar(status.authId, status.profilePictureUrl); }),
    getWeeklyUsage: async () => withService(async (service) => { const status = await service.getStatus(); return status.kind === "logged-in" && status.localProvider == null ? await deps.fetchWeeklyUsage(tokenReader(service)) : null; }),
    getUsageSummary: async () => !await deps.isUsagePageEnabled() ? null : await withService(async (service) => { const status = await service.getStatus(); return status.kind === "logged-in" && status.localProvider == null ? await deps.fetchUsageSummary(tokenReader(service)) : null; }),
    getPrReviewPreferences: async () => withService(async (service) => { const status = await service.getStatus(); return status.kind === "logged-in" && status.localProvider == null ? await deps.fetchPrReviewPreferences(tokenReader(service)) : NO_SAND_PR_REVIEW_PREFERENCES; }),
    getPrivacyModeEnabled: async () => withService(async (service) => { const status = await service.getStatus(); return status.kind === "logged-in" && status.localProvider == null ? await deps.fetchPrivacyModeEnabled(tokenReader(service)) : true; }),
    cancelTrial: async () => !await deps.isUsagePageEnabled() ? { ok: false, message: "This isn’t available right now" } : await withService(async (service) => { const status = await service.getStatus(); return status.kind === "logged-in" && status.localProvider == null ? await deps.cancelTrial(tokenReader(service)) : { ok: false, message: status.kind === "logged-in" ? "Billing is unavailable in local mode" : "Sign in to Cursor to continue" }; }),
    invokeDashboardAction: async (raw: unknown) => {
      ...
      return await withService(async (service) => { const status = await service.getStatus(); return status.kind === "logged-in" && status.localProvider == null ? await deps.invokeDashboardAction(tokenReader(service), request) : { ok: false, message: status.kind === "logged-in" ? "Dashboard actions are unavailable in local mode" : "Sign in to Cursor to continue" }; });
    },
  };
}
