...
const OVERLAY_TONE_SCRIM_RGB = [20, 20, 20] as const;
const OVERLAY_TONE_SCRIM_ALPHA_LIGHT = 0x80 / 255;
const OVERLAY_TONE_SCRIM_ALPHA_DARK = 0xb2 / 255;
const OVERLAY_TONE_TRANSPARENT_COLOR = "rgba(0, 0, 0, 0)";

function isDarkBackground(backgroundHex: string): boolean {
  const [red, green, blue] = parseHexRgb(backgroundHex) ?? [24, 24, 24];
  return (red * 299 + green * 587 + blue * 114) / 1_000 < 128;
}

function overlayToneScrimColor(themeBackground: string): string {
  const scrimAlpha = isDarkBackground(themeBackground)
    ? OVERLAY_TONE_SCRIM_ALPHA_DARK
    : OVERLAY_TONE_SCRIM_ALPHA_LIGHT;
  return blendRgbaOverHex(OVERLAY_TONE_SCRIM_RGB, scrimAlpha, themeBackground);
}

function windowsTitleBarOverlayBackground(themeBackground: string, isOverlayTone: boolean): string {
  return isOverlayTone ? OVERLAY_TONE_TRANSPARENT_COLOR : themeBackground;
}
...
export function windowsTitleBarOverlayFor(
  ...
  return {
    height: windowsTitleBarOverlayHeight(isOverlayTone),
    color: windowsTitleBarOverlayBackground(themeBackground, isOverlayTone),
    symbolColor: titleBarOverlaySymbolColor(
      isOverlayTone ? overlayToneScrimColor(themeBackground) : themeBackground,
    ),
  };
}
