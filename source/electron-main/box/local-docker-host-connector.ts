...
export const LOCAL_DOCKER_API_COMPAT_VERSION = "1.44";
const READY_TIMEOUT_MS = 180_000;
const OPTIONAL_CREDENTIAL_TIMEOUT_MS = 3_000;
const DOCKER_COMMAND_TIMEOUT_MS = 15_000;
const DOCKER_DAEMON_RETRY_DELAYS_MS = [1_000, 2_000, 4_000, 8_000] as const;
...
export function resolveDockerExecutable(env: NodeJS.ProcessEnv = process.env): string {
  const configured = env.DOCKER_PATH?.trim();
  if (configured != null && configured.length > 0) return configured;
  if (process.platform === "win32") {
    const candidates = [
      env.LOCALAPPDATA == null ? undefined : join(env.LOCALAPPDATA, "Programs", "DockerDesktop", "resources", "bin", "docker.exe"),
      env.LOCALAPPDATA == null ? undefined : join(env.LOCALAPPDATA, "Programs", "Docker", "Docker", "resources", "bin", "docker.exe"),
      env.ProgramFiles == null ? undefined : join(env.ProgramFiles, "Docker", "Docker", "resources", "bin", "docker.exe"),
    ];
...
export function resolveDockerDesktopExecutable(env: NodeJS.ProcessEnv = process.env): string | undefined {
  if (process.platform !== "win32") return undefined;
  const candidates = [
    env.LOCALAPPDATA == null ? undefined : join(env.LOCALAPPDATA, "Programs", "DockerDesktop", "Docker Desktop.exe"),
    env.LOCALAPPDATA == null ? undefined : join(env.LOCALAPPDATA, "Programs", "Docker", "Docker", "Docker Desktop.exe"),
    env.ProgramFiles == null ? undefined : join(env.ProgramFiles, "Docker", "Docker", "Docker Desktop.exe"),
  ];
  return candidates.find((candidate): candidate is string => candidate != null && existsSync(candidate));
}

function dockerEnvironment(): NodeJS.ProcessEnv {
  const env = { ...process.env };
  if (process.platform === "win32" && (env.DOCKER_API_VERSION?.trim() ?? "").length === 0) env.DOCKER_API_VERSION = LOCAL_DOCKER_API_COMPAT_VERSION;
  return env;
}
...
function runDocker(args: readonly string[]): Promise<CommandResult> {
  return new Promise((resolve) => {
    ...
    child.once("close", (code) => finish({ ok: code === 0, output: output.trim() }));
    timer = setTimeout(() => {
      child.kill();
      finish({ ok: false, output: `${output}\nDocker command timed out after ${DOCKER_COMMAND_TIMEOUT_MS}ms.`.trim() });
...
function shouldWakeDockerDesktop(output: string): boolean {
  return /cannot connect|daemon is not running|dockerDesktopLinuxEngine|timed out|The system cannot find the file|ENOENT/i.test(output);
}

function wakeDockerDesktop(): boolean {
  const executable = resolveDockerDesktopExecutable();
...
async function ensureDockerDaemonAvailable(): Promise<CommandResult> {
  let result = await runDocker(["info", "--format", "{{.ServerVersion}}"]);
  if (result.ok) return result;
  if (process.platform === "win32" && shouldWakeDockerDesktop(result.output)) wakeDockerDesktop();
  for (const delayMs of DOCKER_DAEMON_RETRY_DELAYS_MS) {
    await wait(delayMs);
    result = await runDocker(["info", "--format", "{{.ServerVersion}}"]);
    if (result.ok) return result;
  }
  return result;
}
...
async function inspectContainer(): Promise<{ exists: boolean; running: boolean; owned: boolean; image: string; hostSha256: string; hasInferenceCredential: boolean; schemaVersion: string }> {
  ...
  if (!result.ok) {
    if (/no such (object|container)/i.test(result.output)) return { exists: false, running: false, owned: false, image: "", hostSha256: "", hasInferenceCredential: false, schemaVersion: "" };
    throw new Error(`Could not inspect the local Docker VM: ${result.output || "Docker did not answer"}`);
  }
  try {
...
export async function getLocalDockerStatus(settingsPath: string): Promise<LocalDockerStatus> {
  const daemon = await ensureDockerDaemonAvailable().catch(() => ({ ok: false, output: "Docker is not installed." }));
  ...
  let inspected: Awaited<ReturnType<typeof inspectContainer>>;
  try { inspected = await inspectContainer(); }
  catch (error) { return { available: true, running: false, ready: false, containerName: LOCAL_DOCKER_BOX_CONTAINER, image: LOCAL_DOCKER_BOX_IMAGE, detail: error instanceof Error ? error.message : String(error) }; }
...
let localRuntimeValidated = false;
...
async function localAuthMountArguments(): Promise<string[]> {
  ...
  for (const [source, destination] of [[codexHome, "/root/.codex"], [join(homedir(), ".claude"), "/root/.claude"]] as const) {
...
async function ensureLocalDockerBox(settingsPath: string, inferenceCredential?: InferenceCredential): Promise<GatewayConnection> {
  ...
  if (localRuntimeValidated && await gatewayReady(token)) return { baseUrl: LOCAL_DOCKER_GATEWAY_URL, token };
  ...
  if (current.exists && !current.running) {
    ...
    const created = await runDocker([
      ...
      "--env", "SAND_SUPERVISOR_ENABLED=1", "--env", "SAND_BOX_AUTO_UPDATE=0", "--env", "SAND_USE_EXISTING_BOX_EXEC_DAEMON=1", "--env", "SAND_LOCAL_AUTH_MOUNT=1", "--env", "SAND_TREE_SITTER_NODE_DEPS=/home/box/deps", "--env", "NODE_PATH=/home/box/deps", "--env", "SAND_GATEWAY_BIND_HOST=0.0.0.0", "--env", "SAND_HOST_PORT=1340", "--env", `SAND_GATEWAY_TOKEN=${token}`,
...
export async function stopLocalDockerBox(settingsPath?: string): Promise<void> {
  ...
  if (settingsPath != null) await rm(inferenceCredentialPath(settingsPath), { force: true });
}

/**
 * Permanently closes local-Docker mutation intake for this process, then stops
 * the owned container. The retained quit path only needs the fail-closed stop.
 */
export async function stopLocalDockerBoxForQuit(settingsPath?: string): Promise<void> {
  await stopLocalDockerBox(settingsPath);
}
