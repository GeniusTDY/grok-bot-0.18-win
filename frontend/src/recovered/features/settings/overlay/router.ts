...
const CLI_PROXY_ROUTER_PROVIDER: RouterProvider = {
  id: "cli-proxy",
...
export const ROUTER_PROVIDER_OPTIONS: readonly RouterProvider[] = [...ROUTER_PROVIDERS, CLI_PROXY_ROUTER_PROVIDER];

const ROUTER_PROVIDER_IDS = new Set<RouterProviderId>(ROUTER_PROVIDER_OPTIONS.map((provider) => provider.id));
...
export function routerProviderById(id: RouterProviderId): RouterProvider {
  return ROUTER_PROVIDER_OPTIONS.find((provider) => provider.id === id) ?? ROUTER_PROVIDERS[0]!;
}
