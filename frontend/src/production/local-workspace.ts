...
import { translateCopy } from "../locale/translate";
...
/**
 * Resolves copy through the injected locale engine. The engine itself is
 * language-agnostic, so the Simplified-Chinese switch is applied here; any copy
 * the engine does not cover falls back to the canonical English string.
 */
function localize(english: string): string {
  if (appLanguage !== "zh") return english;
  return translateCopy(english) ?? english;
}

export function localWorkspaceNextAction(readiness: LocalWorkspaceReadiness): string {
  if (readiness.kind === "checking") {
    return localize("Checking your local 9Router workspace…");
  }
  if (readiness.kind === "ready") {
    return localize("Local 9Router is ready. Continue without signing in.");
  }
  return readiness.blockers[0]?.message
    ?? localize("Finish the Local 9Router setup to continue without signing in.");
}

export function localWorkspaceRemainingBlockerLabel(count: number): string {
  return localize(`${count} setup items remain.`);
}
