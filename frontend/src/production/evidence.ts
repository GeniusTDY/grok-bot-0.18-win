...
const UI_TEXT_ZH: Record<keyof typeof UI_TEXT_EN, string> = Object.fromEntries(
  Object.entries(UI_TEXT_EN).map(([key, english]) => [key, translateCopy(english) ?? english])
) as Record<keyof typeof UI_TEXT_EN, string>;

/**
 * The UI text dictionary. Simplified Chinese is resolved through the injected
 * locale engine (`translateCopy`), so this surface shares a single translation
 * source with the rest of the renderer. Unsupported copy (e.g. the "Grok Bot"
 * product name) falls back to the canonical English string. Simplified Chinese
 * is only used when the Electron system language is Simplified Chinese.
 */
