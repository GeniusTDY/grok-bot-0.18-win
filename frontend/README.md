...
It is built with Vite as a readable design workspace. Packaged builds retain the
checksum-pinned shipped renderer, hydrated by `npm run bootstrap:windows`, and
apply the reconstructed Router settings transform on top of it.

The small files under `manifests/` identify assets and reviewed semantic
boundaries. The upstream renderer itself is not tracked: `npm run bootstrap:windows`
