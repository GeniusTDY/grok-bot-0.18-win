...
import { copyFileTolerant, isLockError, moveTolerant, pathExists, removeTreeTolerant } from "./tolerant-fs.mjs";
...
/**
 * Keeps the current artifact recoverable before the freshly packed one is
 * placed. A locked artifact cannot be renamed away, so fall back to a copy and
 * let the placement step overwrite it in place.
 */
async function backupArtifact(source, backup) {
  try {
  ...
  await copyFileTolerant(source, backup, { preserveTimestamps: true });
}

async function restoreArtifact(backup, target) {
  if (!await pathExists(backup)) return;
  await moveTolerant(backup, target);
}
...
export async function packStagedAppWithIntegrity({
  ...
  try {
    ...
    try {
      if (hadArchive) {
        await backupArtifact(archivePath, previousArchive);
        archivedPrevious = true;
      }
      if (hadUnpacked) {
        await backupArtifact(unpackedRoot, previousUnpacked);
        unpackedPrevious = true;
      }
      await removeTreeTolerant(archivePath);
      await moveTolerant(temporaryArchive, archivePath);
      await removeTreeTolerant(unpackedRoot);
      if (await exists(temporaryUnpacked)) await moveTolerant(temporaryUnpacked, unpackedRoot);
      const result = await verifyStagedPackageIntegrity({ stageRoot, archivePath, unpackedRoot, before });
      await removeTreeTolerant(previousArchive);
      await removeTreeTolerant(previousUnpacked);
      return { ...result, before };
    } catch (error) {
      await removeTreeTolerant(archivePath);
      await removeTreeTolerant(unpackedRoot);
      if (unpackedPrevious) await restoreArtifact(previousUnpacked, unpackedRoot);
      if (archivedPrevious) await restoreArtifact(previousArchive, archivePath);
      throw error;
    }
  } finally {
