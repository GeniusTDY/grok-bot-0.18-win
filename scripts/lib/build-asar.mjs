...
import { copyTreeTolerant, removeTreeTolerant } from "./tolerant-fs.mjs";
import { resolveRuntimeApp, runtimeLayout } from "./runtime.mjs";
...
export async function stageElectronRuntimeDependencyResolution(depsRoot) {
  ...
  for (const packageName of electronRuntimeResolutionPackages) {
    ...
    await removeTreeTolerant(destination);
    await mkdir(path.dirname(destination), { recursive: true });
    await copyTreeTolerant(source, destination, { preserveTimestamps: true });
...
export async function buildAsar({
  ...
  const layout = runtimeLayout(runtimeApp);
  const runtimeUnpacked = path.join(layout.unpacked, "dist");

  await removeTreeTolerant(buildRoot);
  await mkdir(buildRoot, { recursive: true });
  await copyTreeTolerant(sourceAppDir, stageRoot, { preserveTimestamps: true });
  ...
  if (layout.platform === "win32") {
    ...
    stagedPackage.reconstructed = true;
    await writeFile(stagedPackagePath, `${JSON.stringify(stagedPackage, null, 2)}\n`);
  }
  ...
  if (rendererOverride) {
    ...
    await removeTreeTolerant(stagedRenderer);
    await copyTreeTolerant(rendererSource, stagedRenderer, { preserveTimestamps: true });
