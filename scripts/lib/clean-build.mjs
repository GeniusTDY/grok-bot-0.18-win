...
const releaseAsarHash = windowsAsarSha256;
...
export const runtimeComposition = Object.freeze([
  ...
  { runtime: "node-runtime-dependencies", path: "dist/node-deps", mode: "generated-runtime", reason: "Lockfile-pinned Windows x64 N-API parser prebuilds are load-tested before being staged for the local-exec daemon Node runtime." },
  ...
  { runtime: "electron-shell", path: "Grok Bot.exe", mode: "artifact-runtime", reason: "The Windows package reuses the checksum-pinned, ABI-matched Electron 0.18 win32-x64 shell and helper binaries." },
]);
...
export const fidelityRuntimeComposition = Object.freeze(runtimeComposition.map(runtime => (
  runtime.runtime === "renderer" ? Object.freeze({
    ...
    reason: "The exact shipped 0.18 Windows renderer bundle is preserved byte-for-byte and accepted only against its complete embedded SHA-256 inventory.",
  }) : runtime
)));
...
export async function createRendererArtifactProvenance({
  ...
  return {
    ...
    upstreamAppAsarSha256: releaseAsarHash,
    mode: "checksum-pinned-artifact-runtime",
